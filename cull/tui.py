...
def main():
    ...
    @click.option("--older-than", default=90, metavar="DAYS", show_default=True,
                  help="only show caches not touched in N days")
    @click.option("--min-size", default=0, metavar="MB", show_default=True,
                  help="only show caches larger than N megabytes")
    def _main(path, older_than, min_size):
        """devcull TUI — interactive full-screen cache cleaner."""
        root = Path(path).resolve()
        app = CullApp(root=root, older_than=older_than, min_size_mb=min_size)
        app.run()
